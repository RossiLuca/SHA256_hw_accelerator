...
#define COMPATIBLE_STRING	"xlnx,sha256-axi-1.0"

/* information about kernel module */
MODULE_AUTHOR("Luca Rossi");
MODULE_LICENSE("GPL");
MODULE_DESCRIPTION(DRIVER_NAME ": example software driver for SHA256");
...
/* called on device read. read 80 bytes (data, status, H0..H7 registers). */
...
static ssize_t sha256_write(struct file *fp, const char __user *buf, size_t count, loff_t * f_pos)
{
	...
	return sizeof(uint32_t); // return number of written bytes
}
